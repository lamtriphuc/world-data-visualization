...
import { deleteTravelTracker, getTravelTracker, postTravelTracker } from '../services/auth.service'
import { useNavigate } from 'react-router-dom';
import { getAllCountries, getAllCountryNames } from '../services/country.service';
...
const TravelTracker = () => {
    ...
    const updateCountryStatus = async (cca3, status, startDate, endDate, note) => {
        ...
        if (status === "none") {
            if (existing) {
                ...
                setCountryStatus(prev => (
                    prev.filter(c => c.cca3 != existing.cca3)
                ));
                return;
            }
            setShowModal(false);
    ...
    const getStatusLabel = (status) => {
        switch (status) {
            case 'visited': return lang == 'en' ? t('visited') : 'Đã đi';
            case 'bucket': return lang == 'en' ? t('bucket') : 'Muốn đi';
            case 'living': return lang == 'en' ? t('living') : 'Đang sống';
            default: return lang == 'en' ? t('none') : 'Chưa đánh dấu';
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-green-500">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">{t('visited')}</p>
                            <p className="text-3xl font-bold text-gray-800 dark:text-gray-200">{stats.visited}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-300 mt-1">
                                {((stats.visited / countries.length) * 100).toFixed(1)}% {t('world')}
                            </p>
                        </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-yellow-500">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-600 dark:text-gray-300  mb-1">{t('bucket')}</p>
                            <p className="text-3xl font-bold text-gray-800 dark:text-gray-200">{stats.bucket}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-300 mt-1">{t('bucket_list')}</p>
                        </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border-l-4 border-blue-500">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-600 dark:text-gray-300  mb-1">{t('living')}</p>
                            <p className="text-3xl font-bold text-gray-800 dark:text-gray-200">{stats.living}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-300 mt-1">{t('residence')}</p>
                        </div>
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800  border-b">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4 text-sm flex-wrap">
                            <span className="font-semibold text-gray-700 dark:text-gray-300">{t('legend')}:</span>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded" style={{ backgroundColor: TRAVEL_COLORS.visited }}></div>
                                <span>{t('visited')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded" style={{ backgroundColor: TRAVEL_COLORS.bucket }}></div>
                                <span>{t('bucket')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded" style={{ backgroundColor: TRAVEL_COLORS.living }}></div>
                                <span>{t('living')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded" style={{ backgroundColor: TRAVEL_COLORS.default }}></div>
                                <span>{t('none')}</span>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-96 overflow-y-auto">
                    {filteredCountries.map(country => (
                        <div
                            key={country.cca3}
                            onClick={() => {
                                const fullCountry = countries.find(c => c.cca3 === country.cca3);
                                setSelectedCountry({
                                    ...country,
                                    country: fullCountry
                                });
                            ...
                            style={{ borderLeftWidth: '4px', borderLeftColor: getStatusColor(country.status) }}
                        >
                            <div className="flex gap-4">
                                <img src={country.flag} alt="" className='w-15' />
                                <div>
                                    <div className="font-semibold text-gray-800 dark:text-gray-300">{getTranslatedName(country.name)}</div>
                                    <div className="text-sm text-gray-600 dark:text-gray-300">{getStatusLabel(country.status)}</div>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-300">{selectedCountry.name}</h3>
                            <button
                        ...
                        <p className="text-gray-600 dark:text-gray-300   mb-6">{t('select_status_for_this_country')}:</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setSelectedCountry(s => ({ ...s, status: 'visited' }))}
                                    ...
                                    >
                                        ...
                                        <span className="font-semibold text-green-700">{t('visited')}</span>
                                    </button>

                                    <button
                                        onClick={() => setSelectedCountry(s => ({ ...s, status: 'bucket' }))}
                                    ...
                                    >
                                        ...
                                        <span className="font-semibold text-yellow-700">{t('bucket')}</span>
                                    </button>

                                </div>
                                <button
                                    onClick={() => setSelectedCountry(s => ({ ...s, status: 'living' }))}
                                ...
                                >
                                    ...
                                    <span className="font-semibold text-blue-700">{t('living')}</span>
                                </button>
                            </div>

                            {/* --- INPUT DATE --- */}
                            {(selectedCountry.status === "visited" || selectedCountry.status === "bucket") && (
                                <div className="space-y-3">

                                    {/* Ngày bắt đầu */}
                                    <div>
                                        <label className="block text-gray-700 dark:text-gray-300 font-semibold mb-1">
                                            {selectedCountry.status === "bucket" ? t('end_day') : t('start_day')}
                                        </label>
                                        <input
                                            type="date"
                                    ...
                                    {selectedCountry.status === "visited" && (
                                        <div>
                                            <label className="block text-gray-700 dark:text-gray-300 font-semibold mb-1">{t('end_day')}</label>
                                            <input
                                                type="date"
                                                className="w-full border p-2 rounded-lg"
                                                value={endDate}
                            ...
                            {/* --- NOTE --- */}
                            <div>
                                <label className="block text-gray-700 dark:text-gray-300 font-semibold mb-1">{t('note')}</label>
                                <textarea
                                    rows={3}
                                    className="w-full border p-2 rounded-lg"
                                    value={note}
                            ...
                            {selectedCountry.status !== "none" && (
                                <button
                                    onClick={() => updateCountryStatus(selectedCountry.cca3, "none")}
